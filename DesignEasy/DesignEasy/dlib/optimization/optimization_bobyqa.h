...
namespace dlib
{
    ...
    public: bobyqa_failure(const std::string& s):error(s){return;}
    };
...
// ----------------------------------------------------------------------------------------
    ...
    // ----------------------------------------------------------------------------------------
        ...
        ) const
        {
            ...
            fsave = fval[1];
            if (nf < npt) {
		//std::cout<<"Return from BOBYQA because the objective function has been called max_f_evals times." <<std::endl;
		return f;
...
L360:
		...
		return f;
          ...
          //      throw bobyqa_failure("Return from BOBYQA because the objective function has been called max_f_evals times.");
            ...
            if (ntrits > 0) {
                if (vquad >= zero) {
			std::cout<<"Return from BOBYQA because a trust region step has failed to reduce Q."<<std::endl;
			return f;
