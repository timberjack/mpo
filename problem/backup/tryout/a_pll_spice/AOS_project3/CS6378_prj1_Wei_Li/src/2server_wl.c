...
{
     int sockfd, newsockfd, pid, portno = 6660;                                           //CHANGE PORT NUMBER!!!
     socklen_t clilen;
...
{  int n,i,j,flag=0;
   ...
   switch(str[0][0])
     {case '1': if((fp=fopen(str[1],"r"))!=NULL)                  // starting from [1]
                  {n = write(sock,"File exists at net12!\n",32);                          //CHANGE MACHINE NAME!!!
                ...
                n = write(sock,"File created at net12!\n",32);                            //CHANGE MACHINE NAME!!!
                if (n < 0) error("ERROR writing to socket");}
                fclose(fp);
     	          break;	 
     	                       
      case '2': if((fp=fopen(str[1],"r"))==NULL)  // starting from [1]   
      	          {n = write(sock,"Cannot find that file at net12!\n",64);                //CHANGE MACHINE NAME!!!
                   if (n < 0) error("ERROR writing to socket");}
      	        else {for( i = 0; i < 16; i++){                             
      	               for( j = 0; j < 63; j++){                            
      ...
      case '3': if((fp=fopen(str[1],"r"))==NULL)
                   {n = write(sock,"Cannot write to a non-exiting file at net12!\n",64);  //CHANGE MACHINE NAME!!!
                    if (n < 0) error("ERROR writing to socket");}
                      ...
                      for( i = 2; i < 8; i++){
                        ...
                        for( j = 0; j < 63; j++){ 
                           ...
                           str[i][j]= '\0'; flag = 1;break;}}}
                n = write(sock,"File content added at net12!\n",64);                      //CHANGE MACHINE NAME!!!
                if (n < 0) error("ERROR writing to socket");}
                fclose(fp);            	
                break;  	               	          
      default:  break;
