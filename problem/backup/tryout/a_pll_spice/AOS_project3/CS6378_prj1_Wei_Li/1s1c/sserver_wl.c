...
{
     int sockfd, newsockfd, pid, portno = 6662;
     socklen_t clilen;
     ...
     while (1) {
         ...
         if (pid == 0)  {
             ...
             exit(0);}
         else close(newsockfd);                  /* while(0), we never get here */
     } close(sockfd);                            /* end of while */
...
{  int n,i,j,flag=0;
   ...
   switch(str[0][0])
     {case '1': if((fp=fopen(str[1],"r"))!=NULL)                // starting from [1]
                ...
                n = write(sock,"File created!\n",16);
                if (n < 0) error("ERROR writing to socket");}
                fclose(fp);
     	          break;	 
     	                       
      case '2': if((fp=fopen(str[1],"r"))==NULL)  // starting from [1]   
      	          {n = write(sock,"Cannot find that file!\n",32);
                   if (n < 0) error("ERROR writing to socket");}
      	        else {for( i = 0; i < 16; i++){                             
      	               for( j = 0; j < 63; j++){                            
      ...
      case '3': if((fp=fopen(str[1],"r"))==NULL)
                   {n = write(sock,"Cannot write to a non-exiting file!\n",64);
                    if (n < 0) error("ERROR writing to socket");}
                      ...
                      for( i = 2; i < 8; i++){
                        ...
                        for( j = 0; j < 63; j++){ 
                           ...
                           str[i][j]= '\0'; flag = 1;break;}}}
                n = write(sock,"File content added!\n",32);
                if (n < 0) error("ERROR writing to socket");}
                fclose(fp);            	
                break;  	               	          
